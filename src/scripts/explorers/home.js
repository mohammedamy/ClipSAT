/* Canvas explorers for the home page (Plan 5 Phase 5.015, ADR 0039): the hero graph and the
   derivative and Riemann-sum explorers, with their shared presets. Moved verbatim out of
   modules/02-core-app.js; base.njk loads public/js/ex/home.js only on the home page, right
   after engine.js. window.hero stays global for the hero's toggle buttons. */
(function(K){
  "use strict";
  if(!K) return;
  var reduceMotion=K.reduceMotion, drawExplorerOf=K.drawExplorerOf, fmt=K.fmt, Plot=K.Plot, redrawAll=K.redrawAll, wireDataToggle=K.wireDataToggle, renderDataRows=K.renderDataRows, dfdx=K.dfdx, integrate=K.integrate, arrow=K.arrow, _deg=K._deg, _fitView=K._fitView, _clip=K._clip, _set=K._set, _slider=K._slider, _num=K._num;
