...
- **Algebra 2 (`alg2`) MCQ bank in the old inline `fullExamBank` structure inside `index.html`** may still show as empty — the modern `bank-data/alg2.json` pool is populated (1,654 questions, all domains at 50/50/50), but check which structure is actually wired into the live quiz UI for this track if a user reports it's empty; the two systems (`bank-data/*.json` fetched at runtime vs. the legacy inline `fullExamBank`) can diverge.
- **The "Ask Mr. Mohamed" chat button never actually appears on track pages** — its HTML markup (`#chatFab`/`#chatPanel`) only exists in the standalone `index.html`, never got added to `base.njk`. If the user wants the chat feature live on track pages, the markup needs to be added to the `baseNjk` template in `build.js`.
