...
<body class="track-{{ trackId }}">
  ...
  <div id="cloud-auth-modal" role="dialog" aria-modal="true" aria-label="Sign in" onclick="if(event.target===this)closeCloudAuthModal()">
    <div id="cloud-auth-box">
      ...
      <p>Sign in with just your email to save your mistake log, flashcard schedule, and streaks across every device. No password — we email you a one-time link.</p>
      <input type="email" id="cloud-auth-email" placeholder="you@example.com" autocomplete="email">
      <button class="cloud-auth-send" onclick="window.cloudSendMagicLink&&window.cloudSendMagicLink()">Send sign-in link</button>
