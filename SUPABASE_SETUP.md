...
- ~~Teacher/parent view (a `role` column + a "linked students" table — not yet in `schema.sql`)~~ — done, but not with a `role` column: a simpler class-code model instead (`classes`/`class_members` in `schema.sql`, `public/js/teacher-view.js`, the "🏫 Class" nav button). A signed-in user creates a class to get a shareable code, or joins one with a code they were given — no separate teacher/student flag needed. Each roster row also shows a "Last active" signal (from `chapter_visits`) and an expandable "Weakest ▾" list of that student's lowest-accuracy track/domain buckets, not just one summed percentage. ~~Assigning a chapter/mock exam to the roster~~ — also done: `assignments`/`assignment_completions` in `schema.sql` let a teacher assign a track + free-text label to a class, and each student self-reports done/not-done (self-reported, not auto-detected from quiz activity — see the top of `teacher-view.js` for why).
- ~~Free-response auto-grading~~ — done for AI-generated questions (both the numeric and CAS-backed symbolic-equivalence graders: `window.aiqCheckFRQ`/`aiqCheckFRQSymbolic` + `window.ClipSATSymbolicCheck` in `src/scripts/engine.js`, already shipped in `b916a3f`). One real gap closed here: the shorter "genTest" AI path wired `numericAnswer`/`answerExpr` into this grading UI, but "genFullExam" (the full AI-generated exam paper) silently dropped both fields and rendered FRQs with no check UI at all — genFullExam now sets the same `data-numeric-answer`/`data-answer-expr` attributes and renders the same `.aiq-frq-check` markup, so both AI paths grade free-response the same way. Static (non-AI) bank-data quizzes still have no free-response auto-grading — they're MCQ-only; the bank has no free-response question type to grade.
- Predictive exam scores — still open.

### Already-set-up projects: pick up the new `assignments`/`assignment_completions` tables
If your Supabase project was created before this update, re-run `schema.sql` (safe to run again in full — every statement is `create ... if not exists` / `create or replace`) to add the new `public.assignments` and `public.assignment_completions` tables and their RLS policies. This is what backs the "🏫 Class" modal's new per-class "Assignments" section.
