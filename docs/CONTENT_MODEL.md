...
|---|---|---|
| `callout` | `{ style, label?, body, copyLatex? }` | The vast majority of prose content. `style` is one of `def` (definition), `thm` (theorem/rule), `note`, `warn` (often a practice problem), `pf` (proof), `example` (worked example), `tip` (study-strategy tip). All of these share one label+body shape in the real HTML (`class="callout def/thm/note/warn/pf"` — 489 instances sitewide — and a separately-classed but identically-shaped `class="example"` — 398 instances). `style` picks the CSS the template renders, not a class name the content author needs to know. |
