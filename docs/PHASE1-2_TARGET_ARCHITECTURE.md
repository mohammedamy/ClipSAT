...
| **WP5 — shipped** | Extended `course_schema.json` with the bilingual `{en,ar}` fields, `videos[]`, and `explorers[]` designed in §2.3 (documented in the new `docs/CONTENT_MODEL.md`); added `ajv` (justified: replaces "hope the JSON is shaped right," ~130 KB dev-dependency, build-time only) and `scripts/validate-content.js`, wired into `npm run build`/CI. Validates the schema's own `examples` too, so schema and docs can't silently drift apart. `content/` doesn't exist yet — the validator reports "nothing to validate" cleanly rather than failing. | 1 PR | None — validates nothing yet, just wires the gate | `git revert` |
| **WP6 (pilot) — 7 of 9 real chapters migrated, scope corrected mid-flight** | Converting `qud-about` first surfaced that the WP5 schema didn't match real content shape at all (`docs/DECISIONS/0002-blocks-model-not-fixed-fields.md`) — fixed with a v2 `blocks[]` schema, confirmed against real markup (sitewide grep of `callout`/`example`/`formula-cards`/`exam-table` class usage; a further real pattern, a standalone worked-example `fcard`, turned up while converting ch.6/ch.8 and was added as `style: "example-card"`). All 7 of qudrat's non-explorer chapters are now converted and validated: `qud-about`, `qud-arithmetic`, `qud-algebra`, `qud-geometry` (fully bilingual, from the real i18n dictionary), `qud-patterns`, `qud-data`, `qud-word` (English-only in the source — `ar: null` throughout, faithfully, not invented). Verified in a real browser (temporary `/wp6-verify/` route, not linked from nav): all 7 chapters render with correct structure, 202 KaTeX elements render as math (not raw LaTeX text), Arabic renders correctly where it exists and falls back to English where it doesn't, the new `example-card` style renders in the correct `fcard` wrapper. `qud-downloads` is now also handled — `downloads-block.njk` (a real template, `courseMeta.officialLinks[]`/`downloadsInfo` in `_meta.json`), verified rendering 3 correctly-nested cards (the source has an unclosed-`<div>` bug across all 21 tracks that this partial fixes rather than reproduces), both real official Qiyas/ETEC links, and the correct `data-track` attribute for the live worksheet-library JS widget. Its boilerplate copy has **zero** Arabic anywhere in the live site (verified, not assumed) — hardcoded English, not invented Arabic. `qud-practice` (the 50-problem set) is also now handled — real markup turned out to be uniform enough (50 identically-structured `<div class="problem">` items) to extract systematically rather than by hand, with a sanity assertion confirming exactly 50 items numbered 1–50 with no gaps. Modeled as a NEW, separate schema shape (`practiceSet`, `content/{track}/_practice-set.json`) rather than forced into `blocks[]` — it's a flat numbered list, not chapter prose. Verified in a real browser: all 50 problems present in order, correct difficulty distribution (9 Basic/30 Intermediate/11 Advanced, matching source), 161 KaTeX elements render as math, and the page's **existing, unmodified** `toggleSol()` JS correctly shows/hides the new markup's solutions — confirming the new templates integrate with old JS, not just that they look right in isolation. **Only `qud-ratio` and `qud-compare` remain** (bespoke interactive-explorer markup that doesn't fit `blocks[]` — deliberately deferred, see `docs/CONTENT_MODEL.md` for the concrete reasons). Also still open: wiring the new bilingual data into the site's *existing* client-side instant language-toggle JS (`window.i18n`); actually cutting `/qudrat/` over to render from `content/` (the live page remains completely untouched). This is real, verified proof the pattern works on real content across a whole track's prose, downloads, and practice set — not a finished migration. | 4 PRs so far | Medium — realized on real content that the original schema design was wrong before any content shipped on it | `index.html`'s qudrat section + `tracks/qudrat.html` completely untouched; `content/qudrat/` and the new partials are pure additions the live site doesn't read yet |
