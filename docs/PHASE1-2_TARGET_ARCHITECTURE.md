...
| **WP6 (pilot) — partially shipped, scope corrected mid-flight** | Converting `qud-about` (the cleanest, explorer-free qudrat chapter) surfaced that the WP5 schema didn't match real content shape at all (see `docs/DECISIONS/0002-blocks-model-not-fixed-fields.md`) — fixed with a v2 `blocks[]` schema first, confirmed against real markup (sitewide grep of `callout`/`example`/`formula-cards`/`exam-table` class usage), then `qud-about` was faithfully converted (`content/qudrat/_meta.json` + `content/qudrat/qud-about.json`) and rendered through new partials (`src/_includes/partials/chapter.njk` + 4 block-kind partials). Verified in a real browser (temporary `/wp6-verify/` route, not linked from nav): correct EN and AR text, correct 7-row table, correct 4-item card grid, KaTeX rendering the LaTeX (12 elements), correct RTL wrapping. **NOT done:** the other 8 qudrat chapters (2 have bespoke interactive-explorer markup that doesn't fit `blocks[]` — see `docs/CONTENT_MODEL.md` — and 2 are template sections, not chapter content); wiring the new bilingual data into the site's *existing* client-side instant language-toggle JS (`window.i18n`); actually cutting `/qudrat/` over to render from `content/` (the live page is untouched). This is real, verified proof the pattern works on real content — not a finished migration. | 1 PR so far, more needed for full qudrat | Medium — realized on real content that the original schema design was wrong before any content shipped on it | `index.html`'s qudrat section + `tracks/qudrat.html` completely untouched; `content/qudrat/` and the new partials are pure additions the live site doesn't read yet |
